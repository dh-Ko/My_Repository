...
db = pymysql.connect(host="localhost", port=3306, user="root", passwd="0000", db="project", charset='utf8')
cursor = db.cursor()
...
def auto_ledON():
    ...
    if inner >= 1: 
        if led_status == "off":
            ...
            db = pymysql.connect(host="localhost", port=3306, user="root", passwd="0000", db="project", charset='utf8')
            cursor = db.cursor()
            led_sql = "UPDATE devices SET dStatus = 'on' WHERE dName = 'living_room'"
        ...
        if led_status == "on":
            ...
            db = pymysql.connect(host="localhost", port=3306, user="root", passwd="0000", db="project", charset='utf8')
            cursor = db.cursor()
            led_sql = "UPDATE devices SET dStatus = 'off' WHERE dName = 'living_room'"
            cursor.execute(led_sql)
            db.commit()
            db.close()
